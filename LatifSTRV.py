...
def login():
	...
	try:
		...
		cokiew = open(".cookie.txt", "w").write(cookie)
		print('\nLogin Berhasil ')
		get_data()
...
###---[ DUMP KOMTOL ]---###
# MALU LAH LE DAH GEDE GA BISA NGODING
...
def crack(idf,pwx,url,awal):
	...
	print(f"\r [{hh}STRV{P}] {ahir} %s/%s OK:%s CP:%s"%(loop,len(id),ok,cp),end=" ");sys.stdout.flush()
	for pw in pwx:
		try:
